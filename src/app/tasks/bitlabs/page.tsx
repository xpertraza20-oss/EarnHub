...
import { ArrowLeft, Loader2, ExternalLink, ShieldCheck, Activity, Coins } from 'lucide-react';
...
export default function BitLabsPage() {
  ...
  const apiKey = process.env.NEXT_PUBLIC_BITLABS_API_KEY || '';
  const userId = String(user.id);
  // Using standard BitLabs Web integration URL
  const offerwallUrl = `https://web.bitlabs.ai/?token=${apiKey}&uid=${userId}`;
        ...
        <div className="w-full glass-panel rounded-3xl p-8 sm:p-12 border border-dynamic shadow-2xl relative overflow-hidden animate-float">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <div className="w-24 h-24 rounded-full glass flex items-center justify-center mb-6 shadow-[0_0_30px_var(--glow-color)] text-dynamic">
              <Activity size={48} className="drop-shadow-lg text-blue-500" />
            </div>

            <h1 className="text-4xl font-extrabold text-dynamic tracking-tight mb-4">BitLabs Surveys</h1>
            
            <p className="text-dynamic-sec text-lg mb-10 max-w-xl leading-relaxed">
              Complete global surveys matched to your profile and earn high rewards. You will be redirected to our secure partner wall. Once completed, your rewards will be automatically credited to your EarnHub balance.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg mb-10 text-left">
              <div className="glass p-4 rounded-xl border border-dynamic flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-dynamic">Instant Credits</h4>
              ...
              <div className="glass p-4 rounded-xl border border-dynamic flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-dynamic-sec mt-0.5">Trusted by millions globally</p>
                </div>
              </div>
            </div>
